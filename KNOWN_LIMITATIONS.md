...
- MySQL result charset is pinned to UTF-8. Masking decodes each result value as
  ...
  client that moves it elsewhere fails the session closed. A text-protocol
  `SET character_set_results = NULL` — the default MySQL Connector/J (and so
  DBeaver) session-init, which requests each column in its own charset for
  client-side decoding — is rewritten to `utf8mb4` on the backend hop so those
  clients connect; authorization and audit still see the original statement. The
  client then receives results converted to `utf8mb4` rather than each column's
  native charset. Only that exact text-protocol form is rewritten; any other
  results charset — an explicit non-UTF-8 one — fails the session closed, and
